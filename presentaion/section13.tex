...
\begin{frame}[fragile]{Problem: Counting Isolated Grid Clusters}
  \textbf{Scenario:}
  Given a 2D grid of '1's (land) and '0's (water), count the number of isolated land clusters (islands) surrounded by water cardinally.
  
  \vspace{0.3cm}
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{grid = [['1','1','0'],['1','1','0'],['0','0','0']]} $\rightarrow$ \textbf{Output:} \texttt{1}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Problem: Largest Connected Grid Cluster}
  \textbf{Scenario:}
  Find the maximum area of a connected grid cluster (island) in a 2D grid, where area is the number of cardinally connected land cells.
  
  \vspace{0.3cm}
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{grid = [[0, 1], [1, 1]]} $\rightarrow$ \textbf{Output:} \texttt{3}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Problem: Spreading Contagion in Grid}
  \textbf{Scenario:}
  In a 2D grid containing fresh items, contaminated items, and empty spaces, find the minimum time elapsed until all fresh items are contaminated. Spreads cardinally. Return -1 if impossible.
  
  \vspace{0.3cm}
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{grid = [[2,1,1],[1,1,0],[0,1,1]]} $\rightarrow$ \textbf{Output:} \texttt{4}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Problem: Water Flow Boundaries}
  ...
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{heights = [[1,2],[2,1]]} $\rightarrow$ \textbf{Output:} \texttt{[[0,0],[0,1],[1,0],[1,1]]}
  \end{itemize}
\end{frame}
