...
\begin{frame}[fragile]{Problem: Days to Warmer Temperature}
  ...
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{[70, 72, 71, 69, 73, 75]} $\rightarrow$ \textbf{Output:} \texttt{[1, 3, 2, 1, 1, 0]}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Problem: Next Greater Element Search}
  \textbf{Scenario:}
  Given two arrays `array1` and `array2` where `array1` is a subset of `array2`, find the first element in `array2` to the right of each `array1` element that is strictly greater.
  
  \vspace{0.3cm}
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{array1 = [2,4], array2 = [1,2,3,4]} $\rightarrow$ \textbf{Output:} \texttt{[3, -1]}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Problem: Volume of Trapped Water}
  ...
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{[3, 0, 2, 0, 4]} $\rightarrow$ \textbf{Output:} \texttt{7}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Problem: Largest Rectangular Area in Histogram}
  ...
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{[2, 4, 5, 4, 2, 3]} $\rightarrow$ \textbf{Output:} \texttt{12}
  \end{itemize}
\end{frame}
