...
\begin{frame}[fragile]{Problem: Circular Route Fuel Stations}
  \textbf{Scenario:}
  Given gas and cost arrays for circular route fuel stations, find the starting station index from which you can travel around the circuit once without running out of gas.
  
  \vspace{0.3cm}
  \textbf{Examples:}
  \begin{itemize}
    \item \textbf{Input:} \texttt{gas = [1,2,3,4,5], cost = [3,4,5,1,2]} $\rightarrow$ \textbf{Output:} \texttt{3}
  \end{itemize}
\end{frame}
